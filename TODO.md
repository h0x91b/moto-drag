...
- [ ] Реализовать веб-форму гонки (`moto.local`) с вводом имени и кнопками «Занять трек»/«Сброс», тянущую режим и круги из админки.
  - [x] Настроить mDNS/Bonjour, чтобы `moto.local` указывал на ESP32 AP без внешнего DNS.
  - [ ] Добавить captive portal (DNS hijack + probe handlers), чтобы страница автоматически открывалась после подключения к `moto-drag`.
  - [x] SPA-страница гонки: поле имени (сохранение в localStorage), отображение текущего режима/кругов, кнопки старта/сброса и живой таймер.
  - [x] Реализовать back-end логику блокировки трассы на модуле: API флаг «track locked», распространение события всем клиентам и освобождение после финиша/сброса.
  - [x] Адаптировать dev-server и SPA под режим ожидания: «трек занят», лайв-таймер активного пилота, отсутствие возможности занять трек.
  - [x] Перенести ту же логику ожидания на прошивку/реальный API после обкатки на dev-server.
