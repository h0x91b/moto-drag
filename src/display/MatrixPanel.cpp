...
namespace display
{
  namespace
  {
    ...
    // HUB75 pinout female reference (180° rotated from male):
    // [  OE ] [ GND ]
    // [ CLK ] [ LAT ]
    // [  C  ] [  D  ]
    // [  A  ] [  B  ]
    ...
    // [ B1  ] [ GND ]
    // [ R1  ] [ G1  ]
    // █ ← key (left) side when looking at the panel front-on

    // HUB75 pin map tailored for the current wiring harness
    // (avoid GPIO34-39: they are input-only and cannot drive the panel)
    constexpr int8_t kR1 = 16;
    constexpr int8_t kG1 = 17;
    constexpr int8_t kB1 = 18;
    constexpr int8_t kR2 = -1;
    constexpr int8_t kG2 = -1;
    constexpr int8_t kB2 = -1;
    constexpr int8_t kA = 13;
    constexpr int8_t kB = 26;
    constexpr int8_t kC = 27;
    constexpr int8_t kD = 14;
    constexpr int8_t kE = -1;
    constexpr int8_t kLat = 19; // keep LAT off ESP32 strapping pins to avoid boot issues
    constexpr int8_t kOe = 23;
    constexpr int8_t kClk = 25;
  ...
  void initMatrixPanel()
  {
    ...
    matrixReady = true;
    uint16_t solidRed = matrixPanel->color565(255, 0, 0);
    matrixPanel->fillScreen(solidRed);
